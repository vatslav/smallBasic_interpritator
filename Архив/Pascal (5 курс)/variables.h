...
extern bool VariablesAreForbidden;//Запрет на запрос значений переменных.
//Нужен тогда, когда в секции var при объявлении массивов для указания диапазонов
//используются выражения, содержащие в себе константы.
/*

Пример:

const size = 2;
var a:array[1..2*size, 1..size ] of integer;
*/

extern char get_token();
extern int get_exp_as_integer( bool* );
...
void START_SUBSYSTEM_VARIABLES()
{
	...
	//По умолчанию режим работы - свободный: запрос значений переменных разрешён.
	VariablesAreForbidden = false;
}
...
struct variable* get_struct_variable( char* name, char type )
{
  ...
  можно сначала сделать на проверку регистра команду.
  */
...
float* getRealArrayValue( char*name);
...
struct variable getNumberFromVariable( char * name )
{
	...
	if ( VariablesAreForbidden == true )
	{
	    serror("here you can't use variables in expressions. Only constants.");
	    return out;
	}
	...
	//Если мы сюда попадаем, то текущей лексемой является имя переменной-массива, а дальше
	//стоят [ ... ]. Посему возможен вызов getArrayValue ( или getIntegerArrayValue или getRealArrayValue)
	{
		...
		if ( type == INTEGER )
		{
		    ...
		    int* var = getIntegerArrayValue( name );
		...
		else
		{
            ...
            float* var = getRealArrayValue( name );
...
{
	bool get_token_mode = SimpleWork; //Сохраняем режим работы get_token( get_token::SimpleWork)
	SimpleWork = false;//Иначе у нас ничего работать не будет ( подсистема expressions на это заточена )

	VariablesAreForbidden = true;//При объявлении массивов можно использовать только константы
	//переменные можно использовать только с начала работы программы.

    bool ok;
    int r = get_exp_as_integer( &ok );
    if( !ok )
    {
        *result = 0;
        return false;
...
/*
Считать элемент массива ( INTEGER или REAL ).
name - имя массива. Текущей лексемой должны быть лексема перед первой открывающей [
( то бишь имя массива ).

Возвращает указатель на переменную. Если ошибка, возвращает 0.
elem_type - предполагаемый тип элемента.


a[1,1-6] - для двумерных массивов
b[3+3] - для одномерных
*/

void* getArrayValue( char* name, char elem_type )
{

    char type = getArrayBaseType( name );
    if ( type == 0 ) return 0;
    if ( type != elem_type )
    {
        serror("getArrayValue:: bad elem_type");
        return 0;
    }

    struct variable* arr = GLOBALS.ptr;
    struct array_describer* dscr = ( struct array_describer * )(arr->var);
    ...
    bool ok;
    int index1 = get_exp_as_integer( & ok );
    if( !ok ) return 0;

    if ( index1 < dscr->from || index1 > dscr->to )
    {
        serror("index is out of range");
        return 0;
    }

    //Если у нас одномерный массив
    if ( dscr->basetype != ARRAY )
    {
        ...
        if ( type == INTEGER )
        {
            return (int*)( dscr->vars) + ( index1 - dscr->from );
        }
        else
        {
            //printf("%p %d %d\n", dscr->vars, index1, dscr->from);
            return (float*)( dscr->vars) + ( index1 - dscr->from );
        }
    ...
    else //двумерный массив
    {
        ...
        int index2 = get_exp_as_integer( & ok );
        if(!ok) return 0;

        int dimension1_size = dscr->to - dscr->from + 1;
        index1 = index1 - dscr->from;
        dscr = ( struct array_describer *)( dscr->vars);
        if ( index2 > dscr->to || index2 < dscr->from )
        {
            serror("index is out of range");
            return 0;
        }
        ...
        if ( type == INTEGER )
            return (int*)( dscr->vars) + dimension1_size*index1 + ( index2 - dscr->from );
        else
            return (float*)( dscr->vars) + dimension1_size*index1 + ( index2 - dscr->from );
    }

}

/*
Считать элемент массива, хранящего значения типа INTEGER
name - имя массива. Текущей лексемой должны быть лексема перед первой открывающей [
( то бишь имя массива ).
...
int * getIntegerArrayValue( char* name )
{
    int* out = getArrayValue( name, INTEGER );
    if(!out)
    {
        return 0;
    }
...
float * getRealArrayValue( char* name )
{
    float* out = getArrayValue( name, REAL );
    if(!out)
    {
        return 0;
    }
...
/*
Добавить константу name со значением value.
Предполагается, что value - число ( или целое, или дробь ) ( проверка этого возлагается на
вызывающую функцию ). В зависимости от value создаёт константа соответсвующего типа.
*/
bool addConstant( char* name, char* value )
{
  char* p = value;
  bool isReal = false;
  while( *p )
  ...
  char type = ( isReal ) ? REAL: INTEGER;
  if ( !addSimpleVariable( name, type ) ) return false;
  //Теперь указатель GLOBALS.ptr содержит указатель на созданную переменную.
  struct variable* cnst = GLOBALS.ptr;
  cnst -> isConst = 1;
  if ( cnst->type == REAL )
  {
    float * data = (float*)(cnst->var);
    *data = atof( value );
  }
  else if ( cnst->type == INTEGER )
  {
    int * data = (int*)(cnst->var);
    *data = atoi( value );
  }
  else
  {
    serror("variables.h::addConstant::expected type INTEGER or REAL");
    return false;
  }
  return true;
}
...
//Ниженаписанные интерфейсные функции вообще говоря не используются.
char getConstantType( char* name )
{
  struct variable * var = findStructVariable( name );
	...
	if ( var -> isConst == 0 )
	{
		serror("This is a variable. Not a constant");
		return 0;
	}
	return var->type;
}
...
{
  char type = getConstantType( name ); // Это сработает быстро, если предварительно вызывалась getConstantType()
	if ( !type ) return 0;
	if ( type != INTEGER )
	{
		serror("This is not an INTEGER constant.");
		return 0;
	}
...
float* getRealConstant( char* name )
{
  char type = getConstantType( name ); // Это сработает быстро, если предварительно вызывалась getConstantType()
	...
	if ( type != REAL )
	{
		serror("This is not an REAL constant.");
		return 0;
	}
