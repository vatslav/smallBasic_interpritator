...
///@struct каркас для хранения служебных слов и операторов
struct commands
{
    char command[20];
    char tok;
...
///текст прочитанного токена
char token[80];
/**тип токена (т.е. группа токена, все токены не смотря на то что имеют свой унимкальный код,
имеют и группу для удобства обработки - число*/
char token_type;
/**внутреннее представление токена - число. 0, если нет внутреннего представления. типы указаны через define`ы */
char tok;
/**массив, в котором находится интерпритируемая программая, так же указывает на текущий обрабатываемый символ */
char *prog;

/**Сообщает является ли переданный символ "пробельным"
@result Возвращает 1, "пробельный"*/
bool iswhite(char c)
{
  if(c == ' ' || c == '\t' || c == '\n' || c == '\r')
     return 1;
...
@param char* error - текст сообщения, отображаемого пользователю*/
