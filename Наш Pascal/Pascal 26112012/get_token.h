...
/**@function get_token
считывает следующую лексему и переводит указатель prog на начало следующей лексемы. пишет результат в глобальные переменные:
...
Т.е. после вызова get_token() эти переменные изменится, если конешно до этого не было putback()
Лексема по деревенски это символы между 2 Делимитрами, их мы пытаемся интерпритировать. Они состовляют 1 команду языка Паскаль
Совсем на пальцах: get_token() едет(при этом копируя все считанное в свой буфер) по символьна пока не встретит  ". , : ; ( ) [ ] \r \0"
Затем пытается сначала 1)проверить не является ли считанное грубой ошибкой 2) интерпритировать это как служебное слово
3)как переменную или массив 4)если до сюда дошли, то значит ошибка, пользователь ошибкся, сообщает ему об этом.
*/
char get_token()
{
  register char *temp;
